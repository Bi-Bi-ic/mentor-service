...
func (storage *userStorage) GetUserByID(id string) (user models.User, status string) {

	queryStmt := storage.DB.Model(&user).Preload("Skills").Preload("Languages").Preload("OldJobs").Preload("Educations").Preload("TimeLines", "delete_at IS NULL").Where("id = ?", id).First(&user)
...
// GetDataByID ...
func (storage *userStorage) GetDataByID(id string) (user models.User, status repo.Status) {
	var err error
	user.ID, err = uuid.FromString(id)
	uuidIsNil := uuid.Equal(user.ID, uuid.Nil)
	if err != nil || uuidIsNil {
	...
	queryStmt := storage.DB.
		...
		Preload("OldJobs").
		Preload("Educations").
		Preload("TimeLines", "delete_at IS NULL")

	find := queryStmt.First(&user)
	err = find.Error
...
func (storage *userStorage) CreateTimeLine(timeline models.TimeLine, userID uuid.UUID) (result repo.RepoResponse, status repo.Status) {
	...
	queryStmt := storage.DB.Model(&user).Updates(map[string]interface{}{"update_at": time.Now().Unix()})
	err := queryStmt.Association("TimeLines").Append([]models.TimeLine{timeline})
	if err != nil {
		result = repo.RepoResponse{Status: false}
		if err == gorm.ErrRecordNotFound {
			status = repo.NotFound
...
func (storage *userStorage) DeleteTimeLine(timelineID uuid.UUID, userID uuid.UUID) (result repo.RepoResponse, status repo.Status) {
	...
	queryStmt := storage.DB.Model(&timeline).Where("user_id = ?", userID)
	...
	queryStmt = storage.DB.Model(&timeline).Where("user_id = ?", userID).Delete(&timeline)
	err := queryStmt.Error
	if err != nil {
		result = repo.RepoResponse{Status: false}
		if err == gorm.ErrRecordNotFound {
...
func (storage *userStorage) GetAllUsers() (users []models.User, status repo.Status) {
	err := storage.DB.Preload("Skills").Preload("Languages").Preload("OldJobs").Preload("Educations").Preload("TimeLines", "delete_at IS NULL").Find(&users).Error
	if err != nil {
...
func (storage *userStorage) GetFeatureUsers() (users []models.User, status repo.Status) {
	err := storage.DB.Preload("Skills").Preload("Languages").Preload("OldJobs").Preload("Educations").Preload("TimeLines", "delete_at IS NULL").Limit(6).Where("feature <> ?", 0).Find(&users).Error
	if err != nil {
		status = repo.CannotGetAll
	}
	status = repo.Success
